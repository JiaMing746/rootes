...
if [[ "$1" = Magisk ]]; then
    ...
    shift
    [[ ! -x /system/bin/dalvikvm ]] && abort "！/system/bin/dalvikvm命令不存在，从Android5以上已被Google移除"
                ...
                [[ -f "$patched" ]] && echo -e "- 修补boot成功已成功注入Magisk到boot.img\n\n- 开始一键ROOT……"
                    case $Phone_Status in
                        ...
                        if [[ -n $a ]]; then
                            ...
                            echo "- 温馨提示：由于是修补boot注入的ROOT，首次安装这个面具软件，可能需要修复运行环境，如果嫌弃在Magisk Manager里修复慢，可在玩机工具箱，Magisk专区进行快速的一键修复"
                            echo "- 恭喜你ROOT成功、后会有期"
