...
[[ ! -d $lu ]] && mkdir -p $lu
echo "开始执行合并方案修改，注：在当前应用的主题基础上修改，已玩机工具箱修改的优先生效！"
...
Merge_Theme=false
echo "已选择不合并主题方案修改，注：如果玩机工具箱不支持的修改的，剩下的没改的就全是系统默认的"
fi
...
cd $lu
zip -rq ${Export_Directory:=$GJZS/玩机工具箱"$Time"}.mtz ./*
cd $TMPDIR/mtz
zip -rq ${Export_Directory:=$GJZS/玩机工具箱"$Time"}.mtz ./*
echo -e "文件已打包到\n${Export_Directory:=$GJZS/玩机工具箱"$Time"}.mtz"
...
echo -e "严重警告⚠！！！不要把我的$z玩机工具箱专用配置文件去Magisk里或者rec里刷入！\n严重警告⚠！！！不要把我的$z玩机工具箱专用配置文件去Magisk里或者rec里刷入！\n严重警告⚠！！！不要把我的$z玩机工具箱专用配置文件去Magisk里或者rec里刷入！"
rm -rf $TMPDIR $lu2 &>/dev/null
...
sleep 1
echo -e "配置文件使用说明：支持识别常用下载路径，无需你手动移动，也就是说下载完后再启用［玩机工具箱］来重新继续个性化选择安装\n原理通过主题代码实现！不修改系统重要APK，所以通用小米全机型全MIUI版本全安卓版本可用"
