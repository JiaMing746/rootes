...
List=${package//,/\\n}
    ...
    if [ "$Immersion_style" = "1" ]; then
        ...
        echo -e "- 已选择仅在指定应用\n$List里隐藏状态栏"
        settings put global policy_control immersive.status=$package
    elif [ "$Immersion_style" = "5" ]; then
        echo -e "- 已选择仅在指定应用\n$List里隐藏虚拟按键"
        settings put global policy_control immersive.navigation=$package
    elif [ "$Immersion_style" = "6" ]; then
        echo -e "- 已选择仅在指定应用\n$List里隐藏状态栏和虚拟按键"
        settings put global policy_control immersive.full=$package
    elif [ "$Immersion_style" = "7" ]; then
        echo -e "- 已选择不在指定应用\n$List里隐藏状态栏"
        settings put global policy_control immersive.status=apps,-$package
    elif [ "$Immersion_style" = "8" ]; then
        echo -e "- 已选择不在指定应用\n$List里隐藏虚拟按键"
        settings put global policy_control immersive.navigation=apps,-$package
    elif [ "$Immersion_style" = "9" ]; then
        echo -e "- 已选择不在指定应用\n$List里隐藏状态栏和虚拟按键"
        settings put global policy_control immersive.full=apps,-$package
