...
if [[ ! -f $File ]]; then
     ...
     if [[ $suffix = apk ]]; then
          size=`wc -c < "$File"`
          a="cat \""$File"\" | pm install -r -S "$size""$installer" 1>/dev/null"
          eval $a
          result=$?
          if [[ $result = 0 ]]; then
                echo "- $name安装成功"
                ...
                cp -f "$File" $TMP
                a="pm install -r"$installer"\""$TMP/$name"\" 1>/dev/null"
                eval $a
                result=$?
                rm -f "$TMP/$name"
          ...
          size=`wc -c < "$File"`
          a="cat \""$File"\" | pm install -r -S "$size" --apex"$installer" 1>/dev/null"
          eval $a
          result=$?
          ...
          if unzip -l "$File" | fgrep -q 'Android/obb'; then
                echo "- 开始解压obb资源……"
                unzip -o "$File" 'Android/*' -d "$SD_PATH" 1>/dev/null
          ...
          session1="pm install-create -r"$installer""
          ...
          echo "- 已检测到有$Number个被分割的apk文件"
                until [[ $Number -eq 0 ]]; do
                     echo "- 开始推送$Number"
                     apk_list=`unzip -l "$File" | egrep '\.apk'$ | sed -n ${Number}p | awk '/.*\.apk$/{print "unzip -p \"'"$File"'\" \""$4"\" | pm install-write -S "$1" '"$session_id"' '"$Number"'.apk"}'`
                     eval $apk_list 1>/dev/null
                     [[ $? -ne 0 ]] && abort "！推送失败，无法安装$name"
                     Number=$((Number-1))
                done
                echo -e "\n- 开始安装……"
                pm install-commit $session_id 1>/dev/null
                result=$?
                     if [[ $result -eq 0 ]]; then
                          echo "- $name成功安装"
                          [[ $Delete_APK = 1 ]] && rm -f "$File"
