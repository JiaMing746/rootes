...
if [[ "$1" = -status ]]; then
    ...
    if [[ -f $Charging_control || -f $Charging_control2 ]]; then
        echo "支持您的设备"
        if [[ $Stop -lt 100 ]]; then
            description="由「玩机工具箱」生成的自动充电保护模块，大于或等于$Stop停止充电，低于或等于$Start才能允许充电，不需要可随时卸载重启即可恢复。"
        elif [[ $Stop -eq 100 ]]; then
            description="由「玩机工具箱」生成的自动充电保护模块，等于$Stop停止充电，低于或等于$Start才能允许充电，不需要可随时卸载重启即可恢复。"
        fi
