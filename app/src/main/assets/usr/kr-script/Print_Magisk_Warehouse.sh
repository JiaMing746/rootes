...
<!-- START -->
    <action interruptible="false">
        ...
        <params>
            ...
            <param name="Set_Time" title="&#x000A;&#x000A;③　第二屏等待时间，时间单位：分钟" desc="给手机限定正常开机时间或者卡在第二屏时间过久，超过了设置的时间无法开机时就进行自动救砖操作。&#x000A;特别警告：请根据自己手机正常开机时间去设置适当的值，如果设置的比正常开机短会造成无法开机的后果，如果手机一直卡在第一屏方案③会无法生效，只有方案①②才会生效" type="number" required="true" value-sh="grep_prop Set_Time \$Data_Dir/Automatic_brick_rescue.log || echo 1.2"/>
...
<!-- START -->
    <action interruptible="false">
        ...
        <set>. ./Magisk_Module/MIUI/GJZS_Theme_Color.sh GJZS_Theme_Color</set>
            <param name="ChongQi" label="是否刷入成功自动重启系统生效" type="switch" />
            <param name="x" value="无论手动输入Hex颜色代码还是使用调色板，颜色格式必须为：#AARRGGBB，可能不支持深色模式" readonly="readonly" />
            <param name="x2" value="LOGO" readonly="readonly" />
            <param name="logo_type" label="请选择LOGO类型" options-sh="printf 'LSP|LSP风格\nmi|小米200W风格\nCustom|自定义'" value-sh="grep_prop logo_type \$Data_Dir/GJZS_Theme_Color.log"/>
            ...
            <param name="logo_background_data" label="logo背景数据" desc="该数据内容为矢量图的android:pathData数据" placeholder="M0,0h4100v4100h-4100z" value-sh="grep_prop logo_background_data \$Data_Dir/GJZS_Theme_Color.log" type="color" />
